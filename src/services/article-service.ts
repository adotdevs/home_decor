...
function enrichArticleContentBlocksForAlt(
  blocks: unknown,
  ctx: ReturnType<typeof articleToAltContext>,
): unknown {
  if (!Array.isArray(blocks)) return blocks;
  return blocks.map((b) => {
    if (!b || typeof b !== "object") return b;
    const row = b as Record<string, unknown>;
    if (row.type !== "image" || !String(row.content || "").trim()) return b;
    const url = String(row.content);
    const { manualAlt, autoGeneratedAlt } = computeStoredAltPair(
      typeof row.alt === "string" ? row.alt : "",
      url,
      ctx,
    );
    return { ...row, alt: manualAlt, autoGeneratedAlt };
  });
}
...
export async function upsertArticle(payload: Record<string, unknown>) {
  ...
  const altCtx = articleToAltContext({
    title,
    categorySlug,
    subcategorySlug: subcategorySlug ?? "",
    excerpt: String(payload.excerpt ?? ex?.excerpt ?? ""),
    focusKeyword: String(payload.focusKeyword ?? ex?.focusKeyword ?? ""),
  });
  const contentBlocks = enrichArticleContentBlocksForAlt(
    sanitizeContentBlocksImageUrls(payload.contentBlocks),
    altCtx,
  );
  const faqNormalized = normalizeFaq(payload.faq);
  const reading = readingMinutesForArticle({
    contentBlocks,
    faq: faqNormalized,
    excerpt: String(payload.excerpt ?? ex?.excerpt ?? ""),
    title,
    readingTime: typeof ex?.readingTime === "number" ? ex.readingTime : undefined,
  });
  ...
  if ("featuredImage" in payload) {
    ...
    if (fi) {
      setDoc.featuredImage = fi;
      const manual = sanitizeAltText(String(payload.featuredImageAlt ?? ""));
      const auto = buildAutoImageAlt(
        {
          ...altCtx,
          excerptSnippet: String(payload.excerpt ?? ex?.excerpt ?? "").slice(0, 220),
          filenameHint: extractFilenameStem(fi),
        },
        fi,
      );
      setDoc.featuredImageAlt = manual;
      setDoc.featuredImageAutoAlt = auto;
    } else {
      unsetFields.featuredImage = 1;
      unsetFields.featuredImageAlt = 1;
      unsetFields.featuredImageAutoAlt = 1;
    }
  }
