...
import { resolveMediaUploadCardAlt } from "@/lib/image-alt";
...
type UploadOpts = { showInGallery: boolean; galleryCategory?: string; libraryContextTitle?: string };
...
export default function Page() {
  ...
  const [libraryBatchContext, setLibraryBatchContext] = useState("");
  ...
  async function uploadFiles(fileList: FileList | File[], opts: UploadOpts) {
    ...
    try {
      for (const file of files) {
        let contextTitle = opts.libraryContextTitle?.trim();
        let contextCategorySlug: string | undefined;
        if (opts.showInGallery && opts.galleryCategory) {
          const label = GALLERY_CATEGORIES.find((c) => c.id === opts.galleryCategory)?.label ?? opts.galleryCategory;
          contextCategorySlug = opts.galleryCategory;
          contextTitle = contextTitle || `${label} room decor inspiration gallery`;
        }
        await adminUploadMedia(file, {
          showInGallery: opts.showInGallery,
          galleryCategory: opts.galleryCategory,
  ...
  return (
    <div className="rounded-2xl border bg-card p-6 shadow-sm">
      ...
      <div className="mt-6 rounded-2xl border border-border/80 bg-muted/20 p-4">
        <label className="block text-sm">
          <span className="font-medium text-foreground">Batch context for auto alt (optional)</span>
          <textarea
            className="mt-2 min-h-[72px] w-full rounded-xl border bg-background px-3 py-2 text-sm"
            placeholder="e.g. Neutral bedroom styling ideas — improves auto-generated alt when filenames are random"
            value={libraryBatchContext}
            onChange={(e) => setLibraryBatchContext(e.target.value)}
          />
        </label>
        <p className="mt-2 text-[11px] leading-relaxed text-muted-foreground">
          Applied to each file in this drop zone. Leave blank to rely on filenames alone. Manual alt is set per upload from article
          or category editors; bulk library uploads use this headline for Pinterest- and Google Images–friendly descriptions.
        </p>
      </div>
      ...
      <div
        ...
        onDrop={(e) => {
          ...
          void uploadFiles(e.dataTransfer.files, {
            showInGallery: false,
            libraryContextTitle: libraryBatchContext,
          });
        }}
      ...
      >
        <input
          ...
          onChange={(e) => {
            ...
            if (fl) void uploadFiles(fl, { showInGallery: false, libraryContextTitle: libraryBatchContext });
            e.target.value = "";
          }}
        />
      ...
      <section className="mt-10 border-t pt-10" id="gallery-upload">
        ...
        <p className="mt-2 max-w-2xl text-sm text-muted-foreground">
          ...
          under the room you select. Alt text is generated per image from the room plus optional batch context above.
        </p>
        ...
        >
          <input
            ...
            onChange={(e) => {
              const fl = e.target.files;
              if (fl) void uploadFiles(fl, { showInGallery: true, galleryCategory, libraryContextTitle: libraryBatchContext });
              e.target.value = "";
            }}
          />
      ...
      <div className="mt-8 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
        {items.map((m) => (
          <figure key={String(m._id || m.url)} className="overflow-hidden rounded-xl border bg-background shadow-sm">
            ...
            <img
              src={m.url || ""}
              alt={resolveMediaUploadCardAlt(m)}
