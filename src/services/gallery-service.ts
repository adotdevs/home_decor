...
import {
  resolveArticleFeaturedAlt,
  resolveArticleInlineImageAlt,
  resolveDisplayAlt,
  buildAutoImageAlt,
  extractFilenameStem,
} from "@/lib/image-alt";
...
export const getMergedGalleryItems = cache(async function getMergedGalleryItems(): Promise<GalleryItem[]> {
  ...
  try {
    ...
    const articles = await Article.find({ status: "published" })
      .select(
        "slug title featuredImage featuredImageAlt featuredImageAutoAlt contentBlocks categorySlug subcategorySlug excerpt focusKeyword",
      )
      .lean();
    ...
    for (const raw of articles as Record<string, unknown>[]) {
      ...
      if (typeof fi === "string" && fi.trim()) {
        pushImg(fi, resolveArticleFeaturedAlt(raw as Record<string, unknown>));
      }
      ...
      if (Array.isArray(blocks)) {
        for (const b of blocks) {
          ...
          if (block.type === "image" && typeof block.content === "string" && block.content.trim()) {
            const alt = resolveArticleInlineImageAlt(raw as Record<string, unknown>, {
              content: String(block.content),
              alt: typeof block.alt === "string" ? block.alt : "",
              autoGeneratedAlt: typeof block.autoGeneratedAlt === "string" ? block.autoGeneratedAlt : "",
            });
    ...
    const uploads = await MediaUpload.find({ showInGallery: true })
      .select("url alt autoGeneratedAlt galleryCategory imageMetadata")
      .lean();

    for (const raw of uploads as {
      url?: string;
      alt?: string;
      ...
      if (!url) continue;
      const room = normalizeGalleryCategory(raw.galleryCategory);
      const roomLabel = String(raw.galleryCategory ?? "").replaceAll("-", " ");
      const auto =
        typeof raw.autoGeneratedAlt === "string" && raw.autoGeneratedAlt.trim()
          ? String(raw.autoGeneratedAlt)
          : buildAutoImageAlt(
              { galleryRoom: roomLabel, filenameHint: extractFilenameStem(url) },
              url,
            );
      const display = resolveDisplayAlt(raw.alt, auto, { galleryRoom: roomLabel }, url);
      add({
        src: url,
        alt: display,
        category: room,
